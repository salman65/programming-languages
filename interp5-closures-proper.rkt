...
;; Next lesson: symbol renaming
;; Then: syntax sugar
;; Then: let over lambda
...
(struct Closure ([func : Lambda]
                 [env : Env]))

(define-type Value (U Integer Closure))

(: EmptyLambda (-> Symbol Arith Closure))
(define (EmptyLambda p b)
  (Closure (Lambda p b) '()))
...
(define (interp exp env)
  (cond [(Num? exp) (Num-n exp)]
        ...
        [(Lambda? exp)
         (Closure exp env)]
        [(Apply? exp)
         (let* ([fun-pos (Apply-func exp)]
                [param-pos (Apply-param exp)]
                [fun-val (interp fun-pos env)]
                [param-val (interp param-pos env)])
           (if (integer? fun-val)
               (error (format "Expected lambda, got integer ~a" fun-val))
               (let* ([actual-lambda (Closure-func fun-val)]
                      [lambda-param (Lambda-parameter actual-lambda)]
                      [lambda-body (Lambda-body actual-lambda)]
                      [new-environ
                       (cons (Binding lambda-param param-val)
                             (append (Closure-env fun-val) env))])
                 (interp lambda-body new-environ))))]))
...
(module+ test
  ...
  (define fun-environ
    (list
     (Binding 'square (EmptyLambda ;'f
                       'x
                       (Mul 'x 'x)))
     (Binding 'add1 (EmptyLambda ;'g
                     'y
                     (Add (Num 1) 'y)))
     (Binding 'double (EmptyLambda ;'h
                       'x
                       (Add 'x 'x)))))
